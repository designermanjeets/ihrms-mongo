...
import { GQL_EMPLOYEE_PAY_CALCULATE,GQL_EDIT_SALARY,SalarySettingsService,GQL_PAYHEADS } from '../../admin-settings/salary-settings/_services/salary-settings.service';
...
export class SalaryEditComponent implements OnInit {
  ...
  getUserDataById() {
    ...
    this.apollo.query<any, any>({ query: GQL_EMPLOYEE_BY_ID, variables: { query: { limit: 100, eCode: eCode }}})
    .subscribe(val => {
  ...
  onAddEventSalary(event: any) {
    ...
    this.apollo.mutate({ mutation: GQL_EDIT_SALARY, variables: rowData[0]  })
      .pipe(map((res: any) => res?.data.editSalary))
      .subscribe((val: any) => {
        if(val) {
